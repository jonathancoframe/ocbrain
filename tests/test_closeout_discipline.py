...
LIVE_SESSION_CENSUS: tuple[tuple[str | None, str, int], ...] = (
    ...
    ("019f5dea-77b1-7693-a34c-23fead2ce442", "runtime_uuid", 211),
...
LIVE_RUNTIME_CENSUS: tuple[tuple[str, int], ...] = (
    ...
    ("local macOS", 23),  # 12
    ("local-macos", 18),  # 13
    ("local macOS + readonlyprod ClickHouse", 13),  # 14
    ("macos", 12),  # 15
)
...
def test_a_junk_harness_hint_never_wins_the_identity_column(tmp_path, hint):
    ...
    ``fleet_cleanup_audit`` landed in the identity column wearing the
...
def test_is_runtime_session_id_admits_exactly_the_two_machine_shapes():
    ...
    """
    admitted = {
        "019f5dea-77b1-7693-a34c-23fead2ce442": True,
        "f15a38ee73631b3cd5f7d30765c37d5f0245d403": True,
        "ab" * 16: True,
        "fleet_cleanup_audit": False,
        "2026-07-22": False,
        "2026-07-21 personalization headers": False,
        "/root/portfolio_receipt": False,
        "current": False,
        "": False,
        None: False,
    }
...
def test_an_alias_key_with_a_space_is_reachable():
    ...
    through the alias.
    """
    aliases = {"Squirtle Profile": "hermes"}
    assert runtime_family("squirtle profile", aliases=aliases) == "hermes"
    assert runtime_family("squirtle-profile", aliases=aliases) == "hermes"
    assert runtime_family("squirtle profile") == "unknown"
...
def test_the_mining_taxonomy_no_longer_matches_family_tokens_as_substrings():
    ...
    `hermes-runtimes` is a path fragment and `f15a38ee` a profile-hash prefix.
    """
    from procmine.episodes import normalize_runtime
    ...
    assert normalize_runtime("~/.local/share/hermes-runtimes/f15a38ee") == "hermes"
    # 13 live rows. procmine still places them, by "local" -- never by "cli".
    assert normalize_runtime("local macOS + readonlyprod ClickHouse") == (
        "unattributed-local"
    )
    assert normalize_runtime("readonlyprod ClickHouse") == "unknown"
...
_RETRIEVAL_SLUG = "2026-07-16-data-lake-schema"
...
def test_a_retrieval_is_never_refused_for_its_session_label(tmp_path):
    ...
    conn = _core(tmp_path)
    for session in (_RETRIEVAL_SLUG, "/root/portfolio_receipt", "current", None, ""):
...
def test_an_existing_core_gains_the_retrieval_column_before_the_first_read(tmp_path):
    ...
    conn = connect(path)
    init_core_v1(conn)
    conn.execute("ALTER TABLE retrieval_uses DROP COLUMN session_id_source")
    conn.commit()
    with pytest.raises(sqlite3.OperationalError):
