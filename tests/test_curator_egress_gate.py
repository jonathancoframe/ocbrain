...
selftest must report an allow-list that admits every policy the corpus contains
as a defect, rather than reporting 240 clean audits.
"""
...
def test_the_audit_records_what_was_declared_beside_what_was_present(tmp_path: Path) -> None:
    ...
    partition = partition_evidence(
        ...
        limit=50,
        project=PROJECT,
        egress_policies=["hosted_ok", "local_only", "approval_required"],
    ...
    assert context["declared_egress_policies"] == [
        "approval_required",
        "hosted_ok",
        "local_only",
    ]
    assert context["present_egress_policies"] == ["hosted_ok", "local_only", "prohibited"]
...
def test_allowlist_is_vacuous_reads_the_reachability_not_the_outcome() -> None:
    assert allowlist_is_vacuous(["hosted_ok", "local_only"], ["hosted_ok", "local_only"]) is True
    assert allowlist_is_vacuous(["hosted_ok"], ["hosted_ok", "local_only"]) is False
...
def test_a_declared_acknowledgement_downgrades_but_never_hides(
    ...
    _config(
        ...
        {
            "egress_policies": ["hosted_ok", "local_only", "approval_required"],
            "egress_allowlist_ack": "single-operator machine; the curator reads my own notes",
        },
    )
    metric = _metric(conn)
    assert metric["value"] == 0.0
