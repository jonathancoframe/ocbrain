...
3. `tool:process` fails 644 of 2177 calls (30%) across 97 sessions; the dominant failure class is `error` (644). Most common message (33x): "{"status": "timeout", "command": "uv run --with paramiko python -c 'import paramiko,time,sys; host=\"<ip>\"; user=\"<user>\"; keys=[\"<pat". The recurring next move is `bash:ssh -o <chain>` (29 times, succeeding 97%).
